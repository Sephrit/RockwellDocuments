...
6. **Verify** — Run `python3 verify_pdfs.py` (install `pypdf` if needed). If a new PDF fails verification, delete it, mark the backlog item `"status": "failed"` with a note, and try the next pending item instead.
...
The existing GitHub Action (`.github/workflows/update-index.yml`) regenerates `README.md` when PDFs change on `main`. You can also run `python3 update_index.py` locally.
